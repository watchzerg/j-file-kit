...
| `RawFilePipeline` | `application/raw_pipeline/pipeline.py` | 第一层三阶段编排：1→`files_misc`；2→`phase2.py` 调度，`phase2_*` 实现；3→`files_misc` 第一层分流到 `files_compressed` / `files_pic` / `files_audio`（视频占位） |
