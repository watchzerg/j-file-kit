...
│   │       ├── raw_pipeline.py      # RawFilePipeline：Raw 管道（当前为空统计占位）
...
**媒体树**（与 YAML 中目录字段对应）：挂载到 **`/media`**。JAV 业务目录在 **`/media/jav_workspace`** 下；**Raw 收件箱整理**在 **`/media/raw_workspace`** 下。各自任务配置中的路径须为对应 **`JAV_MEDIA_ROOT` / `RAW_MEDIA_ROOT`** 的子路径（Pydantic 校验）。
...
JAV 整理链路的**字段级与分支级说明**（`analyze_jav_file` 顺序、番号、`dry_run`）见 **[JAV_VIDEO_PROCESSING_PIPELINE.md](./JAV_VIDEO_PROCESSING_PIPELINE.md)**；Raw 占位与后续规划见 **[RAW_FILE_PROCESSING_PIPELINE.md](./RAW_FILE_PROCESSING_PIPELINE.md)**；本节为全局骨架。
...
4. **`JavVideoOrganizer.run`**：校验 **`inbox_dir`** → **`JavAnalyzeConfig`** → **`FilePipeline.run`**。**`RawFileOrganizer.run`**：校验 **`inbox_dir`** → **`RawAnalyzeConfig`** → **`RawFilePipeline.run`**（当前返回空统计）。  
5. **`FilePipeline`**：**深度优先**遍历 `scan_root` → 每文件 **`analyze_jav_file`** → **`execute_decision`**（dry_run 时执行器仍为预览路径）→ **`save_result`**；收尾 **`get_statistics`** → **`FileTaskRunStatistics`**。  
...
| `RawFilePipeline` | `application/raw_pipeline.py` | Raw 管道占位：`run` 返回空统计；后续第一层扫描 |
