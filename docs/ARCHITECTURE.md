...
| `RawFilePipeline` | `application/raw_pipeline/pipeline.py` | 第一层三阶段编排：1→`files_misc`；2→`phase2.py` 调度，`phase2_*`（2.2 有小目录体积门禁）；3→`files_misc` 单层 3.0 关键字预删后分流至 `files_compressed` / `files_pic` / `files_audio`（视频占位） |
