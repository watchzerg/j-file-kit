...
│   │   ├── connection.py         # 连接管理
│   │   ├── schema.py             # 表结构初始化
│   │   └── file_task/            # file_task 领域仓储实现
│   ├── persistence/yaml/         # YAML 仓储实现（任务配置）
│   │   └── default_file_task_config_initializer.py  # 默认配置初始化
...
└── application/
    ├── schemas.py                # 请求/响应 DTO（可按职责拆为 config_schemas.py 等）
    ├── config.py                 # 任务配置模型
    ├── config_validator.py       # 配置验证（纯函数）
...
5. **单实例部署**：FileTaskRunManager 约束同一 task_type 同时仅允许一个执行实例，基于单进程部署假设
...
├── api/                                      # 镜像 src/j_file_kit/api/
│   └── conftest.py                           # create_app(base_dir=tmp_path)
...
└── infrastructure/
    ...
    └── persistence/
        ├── sqlite/file_task/                 # SQLite 仓储集成测试
        └── yaml/                             # YAML 仓储集成测试
```
...
| `tests/api/conftest.py` | `create_app(base_dir=tmp_path)` 等 API 测试 fixtures |
