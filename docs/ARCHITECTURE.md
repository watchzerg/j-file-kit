...
│   │       ├── pipeline.py              # FilePipeline：扫描调度 + 任务生命周期（编排）
│   │       ├── pipeline_item_processor.py  # 单文件：analyze_jav_file → execute_decision → 落库
│   │       ├── pipeline_result_mapper.py   # build_file_item_data（Decision + Result → FileItemData）
