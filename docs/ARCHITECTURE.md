...
│   │       ├── raw_pipeline.py      # RawFilePipeline：inbox 第一层三阶段编排（阶段 1 归集 files_misc；2/3 占位）
...
4. **`JavVideoOrganizer.run`**：校验 **`inbox_dir`** → **`JavAnalyzeConfig`** → **`FilePipeline.run`**。**`RawFileOrganizer.run`**：校验 **`inbox_dir`** → **`RawAnalyzeConfig`** → **`RawFilePipeline.run`**（第一层文件/目录三阶段；阶段 1 写 **`file_results`**，收尾 **`get_statistics` 与 `phase*` 计数合并**为 **`FileTaskRunStatistics`**）。  
...
| `RawFilePipeline` | `application/raw_pipeline.py` | inbox 第一层三阶段：阶段 1 → `files_misc`；2/3 占位；合并仓储统计与 `phase*` |
