...
def generate_new_filename(
    ...
    将文件名分解为4部分，按业务规则重构文件名。
    
    Args:
        original_path: 原文件路径
        serial_id: 番号（标准化格式：字母-数字）
        pattern: 番号正则表达式
    ...
    Examples:
        >>> generate_new_filename(Path("video_ABC-001_hd.mp4"), "ABC-001", pattern)
        Path("ABC-001 video-serialId-hd.mp4")
        
        >>> generate_new_filename(Path("abc-001_video.mp4"), "ABC-001", pattern)
        Path("ABC-001 video.mp4")
        
        >>> generate_new_filename(Path("ABC-001.mp4"), "ABC-001", pattern)
        Path("ABC-001.mp4")
    """
