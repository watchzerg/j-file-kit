...
class ProcessorResult(BaseModel):
    ...
    @classmethod
    def success(cls, message: str = "", data: dict[str, Any] | None = None) -> ProcessorResult:
        """创建成功结果"""
        return cls(
    ...
    @classmethod
    def skip(cls, message: str = "", data: dict[str, Any] | None = None) -> ProcessorResult:
        """创建跳过结果"""
        return cls(
    ...
    @classmethod
    def warning(cls, message: str, data: dict[str, Any] | None = None) -> ProcessorResult:
        """创建警告结果"""
        return cls(
...
class TaskResult(BaseModel):
    ...
    processor_results: list[ProcessorResult] = Field(default_factory=list, description="各处理器结果")
    ...
    def has_errors(self) -> bool:
        """是否有错误"""
        return any(result.status == ProcessorStatus.ERROR for result in self.processor_results)
    
    @property
    def has_warnings(self) -> bool:
        """是否有警告"""
        return any(result.status == ProcessorStatus.WARNING for result in self.processor_results)
    
    @property
    def was_skipped(self) -> bool:
        """是否被跳过"""
        return any(result.status == ProcessorStatus.SKIP for result in self.processor_results)
