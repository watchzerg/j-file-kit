...
class TaskStatisticsFinalizer(Finalizer):
    ...
    def finalize(self) -> ProcessorResult:
        ...
        try:
            ...
            # 获取文件统计和性能指标
            detailed_stats = self.file_result_repository.get_detailed_statistics()

            # 合并统计信息为统一结构
            statistics: dict[str, Any] = {
                "operations": {
                    "by_operation_type": operation_stats.get("by_operation_type", {}),
                    "by_file_type": operation_stats.get("by_file_type", {}),
                },
                "files": {
                    "by_file_type": detailed_stats.get("by_file_type", {}),
                },
            ...
            return ProcessorResult.success(
                ...
                f"文件数={sum(f.get('total', 0) for f in detailed_stats.get('by_file_type', {}).values())}"
            )
