"""Raw 管道阶段 2：inbox 第一层目录（关键字迁出 / 清洗 / 单链折叠 / 按类型分类）。
...
def _ensure_phase2_classification_targets(cfg: RawAnalyzeConfig) -> None:
...
def _media_kind_flat(suffix: str, cfg: RawAnalyzeConfig) -> str | None:
    """小目录拆解判定：仅识别图片/视频/音频/压缩；其余扩展名返回 None（一票否决拆解）。"""
    ext = suffix.lower()
...
def _should_flatten_small_dir(files: list[Path], cfg: RawAnalyzeConfig) -> bool:
...
def _whole_dir_destination_for_kinds(
    ...
    """返回整目录迁入的根路径与计数桶标签（pic/audio/compressed/video/misc）。"""
    pic = cfg.folders_pic
...
def _move_whole_classified_dir(
    ...
    *,
    ...
    """将目录树整体迁入某个 folders_*（按树下全部文件的扩展名画像决定桶）。"""
...
def _flatten_dir_into_misc(
    ...
    *,
    ...
    """拆解单层目录：文件迁入 files_misc，必要时合并目录名与 stem；成功后删除空目录。
...
def _run_phase2_classify(
    ...
    *,
    ...
    """阶段 2.4：先尝试小目录拆解，否则整目录归入 folders_*。
    ...
    if _should_flatten_small_dir(files, ctx.analyze_config):
        return _flatten_dir_into_misc(
            ctx,
            dir_path,
            phases,
            dry_run=dry_run,
...
def _phase2_process_one_level1_dir(
    ...
    *,
    ...
    cancelled_classify = _run_phase2_classify(
        ctx,
        dir_for_classify,
        phases,
        dry_run=dry_run,
...
def run_phase2(
    ...
    *,
    ...
    if needs_classification_targets:
        _ensure_phase2_classification_targets(ctx.analyze_config)
